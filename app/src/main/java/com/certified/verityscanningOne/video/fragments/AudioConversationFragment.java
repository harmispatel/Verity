...
public class AudioConversationFragment extends BaseConversationFragment implements CallActivity.OnChangeDynamicToggle {
    ...
    protected void configureActionBar() {
        ...
        if (currentUser != null) {
            if (currentUser.getTags().size() == 0) {
                actionBar.setTitle("Ve");
                CommonSession mCommonSession = new CommonSession(getActivity());
                actionBar.setSubtitle(String.format(getString(R.string.subtitle_text_logged_in_as), mCommonSession.getLoggedUserFName() +
                        " " + mCommonSession.getLoggedEmail()));
            } else {
    ...
    int getFragmentLayout() {
        return R.layout.video_fragment_audio_conversation;
    }
